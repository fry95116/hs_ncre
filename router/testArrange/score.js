...
(function () {
    ...
    router.get('/', function (req, res, next) {
        dbo.selectScore(req.query)
            ...
            .catch(function (err) {
                req.log.error('分数_获取_失败',{err:err});
        ...
        if (req.file) {
            ...
            if(reader){
                reader(req.file.path)
                    .then(function(){
                        req.log.info('分数_导入_成功');
                        res.send('导入成功');
                    })
                    .catch(function(err){
                        req.log.error('分数_导入_失败',{err:err});
    ...
    router.put('/:id_number/:field', bodyParser.urlencoded({extended: true}), function (req, res, next) {
        if (req.body && req.body.value) {
            ...
            dbo.updateScore(req.params.id_number, newData)
                .then(function () {
                    req.log.info('分数_修改_成功',{id_number:req.params.id_number});
                    res.send('修改成功');
                })
                .catch(function (err) {
                    req.log.error('分数_修改_失败',{err:err,id_number:req.params.id_number});
    ...
    router.delete('/', function (req, res, next) {
        dbo.deleteAllScore()
            .then(function () {
                req.log.info('分数_清空_成功');
                res.send('删除成功');
            })
            .catch(function (err) {
                req.log.error('分数_清空_失败',{err:err});
    ...
    router.delete('/:id_number', function (req, res, next) {
        dbo.deleteScore(req.params.id_number)
            .then(function () {
                req.log.info('分数_删除_成功',{id_number:req.params.id_number});
                res.send('删除成功');
            })
            .catch(function (err) {
                req.log.error('分数_删除_失败',{err:err,id_number:req.params.id_number});
                res.status(400).send('删除失败：' + err.message);
            })
    });
