...
(function () {
    var router = require('express').Router(),
        ...
        domainName = user_config.domainName,
                ...
                if(result.total === 0)
                    ...
                    updateID.create(req.query.id_number)
                        .then(function(req_id){
                            ...
                            //     .then(function(){
                            //         res.redirect('updateEnterInfo/mailSended');
                            ...
                            res.render('frontStage/op_res', {
                                isPreview:true,
                                content:'修改链接：' +
                                '<a href="/updateEnterInfo?id_number=' + req.query.id_number + '&req_id=' + req_id + '">' +
                                domainName + '/updateEnterInfo?id_number=' + req.query.id_number + '&req_id=' + req_id +
                                '</a>' +
                                '请点击修改链接完成接下来的修改操作。'
                        ...
                        .catch(function(err){
                            if(err.code === 'EEXIST'){
                                var second = parseInt(err.message);
                                var time = second % 60 === 0 ? '' + second + '秒' : '' + Math.floor(second / 60) + '分' + second % 60 + '秒';
                                res.render('frontStage/op_res',{isPreview:true,content:'验证邮件获取失败,请于' + time + '后再次尝试修改'});
                            }
                            else{
                                req.log.error('报名信息_获取链接_失败',{err:err});
                                res.render('frontStage/op_res',{isPreview:false,content:'unknown'});
    ...
    /** 验证邮件已发送 */
    router.get('/requestUpdateEnterInfo/mailSended',function(req,res,next){
        res.render('frontStage/op_res',{isPreview:false,content:'updateEnterInfo/mailSended'});
    });
    ...
    router.post('/updateEnterInfo',bodyparser.urlencoded({extended: true}),function (req, res) {
        if(_.isUndefined(req.query.id_number) ||_.isUndefined(req.query.req_id)) next();
            ...
            updateID.check(req.query.req_id,req.query.id_number)
                ...
                .then(_.partial(updateID.remove,req.query.req_id,req.query.id_number))
                .then(function(){
                    //日志
                    ...
                    if(result.total === 0)
                        ...
                        updateID.create(req.query.id_number)
                            .then(function(req_id){
                                ...
                                //     .then(function(){
                                //         res.redirect('/requestUploadPhoto/mailSended');
                                ...
                                res.render('frontStage/op_res', {
                                    isPreview:true,
                                    content:'上传链接：' +
                                    '<a href="/uploadPhoto?id_number=' + req.query.id_number + '&req_id=' + req_id + '">' +
                                    domainName + '/uploadPhoto?id_number=' + req.query.id_number + '&req_id=' + req_id +
                                    '</a>' +
                                    '请点击上传链接完成接下来的照片上传操作。'
                                });
                            ...
                            .catch(function(err){
                                if(err.code === 'EEXIST'){
                                    var second = parseInt(err.message);
                                    var time = second < 60 ? '' + second + '秒' : '' + Math.floor(second / 60) + '分' + second % 60 + '秒';
                                    res.render('frontStage/op_res',{isPreview:true,content:'验证邮件获取失败,请于' + time + '后再次尝试上传'});
                                }
                                else{
                                    req.log.error('上传照片_获取链接_失败',{err:err});
                                    res.render('frontStage/op_res',{isPreview:false,content:'unknown'});
    ...
    /** 验证邮件已发送 */
    router.get('/requestUploadPhoto/mailSended',function(req,res,next){
        res.render('frontStage/op_res',{isPreview:false,content:'uploadPhoto/mailSended'});
    });
