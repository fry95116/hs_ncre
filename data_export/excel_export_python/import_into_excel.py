...
records = pyexcel.get_records(file_name="ImportTemplate - 副本.xlsx",name_columns_by_row = 0)

for record in records:
    if(record['考点代码']):
        # 有的行会是空的，将其跳过
        data = (int(record['考点代码']), record['考生姓名'], mapping_sex[record['性别']], record['出生日期'], int(record['证件类型']), str(record['证件号']), int(record['民族']), int(record['职业']), int(record['文化程度']), int(record['培训类型']), int(record['考试科目代码']), record['邮编'], record['地址'], record['电子邮件'], record['联系电话'], record['备注'])
